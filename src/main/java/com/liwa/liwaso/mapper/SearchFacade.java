...
import com.liwa.liwaso.datasource.*;
...
public class SearchFacade {
    ...
    public SearchVO searchAll(@RequestBody SearchRequestDTO searchRequestDTO, HttpServletRequest request) {
        String type = searchRequestDTO.getType();
        SearchTypeEnum searchTypeEnum = SearchTypeEnum.getEnumByValue(type);
        ThrowUtils.throwIf(StringUtils.isBlank(type), ErrorCode.PARAMS_ERROR);
        String searchText = searchRequestDTO.getSearchText();
        long current = searchRequestDTO.getCurrent();
        long pageSize = searchRequestDTO.getPageSize();
        // 搜索出所有数据
